...
from AutoLioNets import AutoLioNet
...
class LioNet:
    ...
    def __init__(self, predictor, encoder, train_data, decoder="auto", time=None, target_scaler=None, feature_names=None, decoder_lower_threshold=0, double_detector=False):
        ...
        self.time = time
        if decoder == "auto":
            self.autolionet = AutoLioNet(self.encoded_training_data, self.train_data, self.time)
            self.decoder = self.autolionet.load_decoder()
        else:
            self.decoder = decoder
        self._extract_feature_statistics()

    def print_examples(self, X_train, threshold):
        self.autolionet.print_examples(X_train, threshold)
        
    def load_best_decoder(self, method):
        # 0 for classicML  
        # 1 for simpleNN
        # 2 for autokeras
        # 3 for autopytorch
        decoder = self.autolionet.reload_decoder(method)
        return decoder
