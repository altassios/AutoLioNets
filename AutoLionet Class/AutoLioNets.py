...
class AutoLioNet:
    ...
    def _autopytorch_strategy(self, encoded_x_train, x_train, time):
        ...
        if time is None:
            ...
            results = autonetR2.fit(X_train=encoded_x_train, Y_train=x_train, X_valid=encoded_x_valid, Y_valid=x_valid, 
                             refit=True)
            return autonet
        #Low time category
        if time == "low":
            autonetR = AutoNetRegression(config_preset="tiny_cs", result_logger_dir="logs/", log_level='info',
                             optimize_metric = 'mean_abs_error', budget_type='time', min_budget=60,
                             max_budget=90, max_runtime=600)
            results_fit = autonetR.fit(X_train=encoded_x_train, Y_train=x_train, X_valid=encoded_x_valid, Y_valid=x_valid, 
                             refit=True)
            return autonetR
        #Medium time category
        if time == "medium":
            autonetR1 = AutoNetRegression(config_preset="medium_cs", result_logger_dir="logs1/", log_level='info',
                             optimize_metric = 'mean_abs_error', budget_type='time', min_budget=120, 
                             max_budget=150, max_runtime=1800)
            results_fit1 = autonetR1.fit(X_train=encoded_x_train, Y_train=x_train, X_valid=encoded_x_valid, Y_valid=x_valid, 
                             refit=True)
            return autonetR1
        #High time category
        if time == "high":
            autonetR2 = AutoNetRegression(config_preset="full_cs", result_logger_dir="logs2/", log_level='info',
                             optimize_metric = 'mean_abs_error', budget_type='time', min_budget=150, 
                             max_budget=180, max_runtime=3600)
            results_fit2 = autonetR2.fit(X_train=encoded_x_train, Y_train=x_train, X_valid=encoded_x_valid, Y_valid=x_valid, 
                             refit=True)
            return autonetR2